...
namespace Unity.FPS.AI
{
    ...
    public class EnemyController : MonoBehaviour
    {
        ...
        // -------------------- LOOT (UPDATED) --------------------
        [Header("Loot")]
        [Tooltip("Список возможных префабов лута. Один из них будет выбран случайно при выпадении.")]
        public List<GameObject> LootPrefabs = new List<GameObject>();

        [Tooltip("Необязательные веса выпадения (чем больше, тем чаще). Длина должна совпадать с LootPrefabs. " +
                 "Если список пуст или длина не совпадает — выбор равновероятный.")]
        public List<float> LootWeights = new List<float>();

        [Tooltip("Общая вероятность выпадения лута (0..1). Если проверку прошёл — выбирается один префаб из списка.")]
        ...
        void OnDie()
        {
            ...
            // ---- RANDOM LOOT ----
            if (TryGetRandomLoot(out var lootToDrop))
            {
        ...
        // ---------- NEW LOOT LOGIC ----------
        /// <summary>
        /// Пытается получить случайный префаб лута из списка, учитывая DropRate и (опционально) веса.
        /// </summary>
        public bool TryGetRandomLoot(out GameObject lootPrefab)
        {
            lootPrefab = null;

            if (DropRate <= 0f || LootPrefabs == null || LootPrefabs.Count == 0)
                return false;

            if (Random.value > DropRate)
                return false;

            // если веса корректны — рулетка
            if (LootWeights != null && LootWeights.Count == LootPrefabs.Count)
            {
                float total = 0f;
                for (int i = 0; i < LootWeights.Count; i++)
                    total += Mathf.Max(0f, LootWeights[i]);
                ...
                if (total > 0f)
                {
                    ...
                    {
                        acc += Mathf.Max(0f, LootWeights[i]);
                        if (r <= acc)
                        {
                            lootPrefab = LootPrefabs[i];
            ...
            // равномерный выбор
            int idx = Random.Range(0, LootPrefabs.Count);
            lootPrefab = LootPrefabs[idx];
            return lootPrefab != null;
        }
        // -----------------------------------

        // (Оставлено для совместимости — больше не используется внутри класса)
        public bool TryDropItem()
        {
            if (DropRate == 0 || LootPrefabs == null || LootPrefabs.Count == 0)
                return false;
            else if (DropRate == 1)
                return true;
            else
